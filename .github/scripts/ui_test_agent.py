...
def sanitize_for_prompt(text: str, max_length: int = 10000) -> str:
    """AI 프롬프트에 삽입되는 텍스트 sanitize"""
    if not text:
        return ""

    # 1. 프롬프트 인젝션 패턴 제거
    injection_patterns = [
        r"(?i)(ignore|forget|disregard).{0,30}(above|previous|instruction|prompt)",
        r"(?i)\[SYSTEM\]|\[INST\]|\[\/INST\]",
        r"(?i)<\|im_start\|>|<\|im_end\|>",
        r"(?i)(system|assistant)\s*:",
        r"(?i)you are now|act as|pretend to be",
        r"(?i)reveal|expose|print|output.{0,20}(key|token|secret|password)",
